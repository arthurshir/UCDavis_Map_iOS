...
class LabelMarker: UIView {
    ...
    class func largeMarkerViewWithText(text: String) -> LabelMarker {
        let lm = LabelMarker()
        ...
        lm.titleLabel.font = UIFont(name: "Avenir-Medium", size: 14)
        ...
        lm.frame = CGRectMake(0, 0, lm.titleLabel.frame.width + 12, 33)
        lm.rectView.frame = CGRectMake(0, 0, lm.titleLabel.frame.width + 8, 25)
        ...
        lm.bottom.frame = CGRectMake(0, lm.frame.height-12, 12, 12)
    ...
    class func largeMarkerWithText(text: String) -> BuildingMarker {
        let marker = BuildingMarker()
        marker.appearAnimation = kGMSMarkerAnimationPop
        marker.icon = Functions.imageWithView( largeMarkerViewWithText(text) )
        marker.title = "🚶"
        return marker
    }
